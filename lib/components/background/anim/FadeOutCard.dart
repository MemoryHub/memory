...
import 'package:memory/components/EmptyCard.dart';
...
class FadeOutCard extends StatefulWidget {
  final List<MemoryInfo> dataList; // 数据
  final int animTime; // 动画时长
  final int displayTime; // 显示时长

  // animTime 默认动画时长为2m,displayTime 默认显示时长为3
  const FadeOutCard({super.key, required this.dataList,this.animTime = 2,this.displayTime = 3});

  @override
  _FadeOutCardState createState() => _FadeOutCardState(dataList,displayTime, animTime);
}
...
class _FadeOutCardState extends State<FadeOutCard>
  ...
  late List<MemoryInfo> _dataList; // 数据
  late int _animTime; // 动画时长
  late int _displayTime; // 显示时长
  Timer? _timer; // 定时器

  _FadeOutCardState(List<MemoryInfo> dataList,int displayTime, int animTime) {
    _dataList = dataList;
    _animTime = animTime;
    ...
    _builderList.add(AnimatedBuilder(
      ...
      builder: (BuildContext context, Widget? child) {
        return Opacity(
            ...
            child: MemoryCard(data: _dataList[index]));
      },
    ));
  }
